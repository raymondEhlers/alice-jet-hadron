...
import JetHUtils
from JetHUtils import EventPlaneAngle
...
class JetHResponseMatrixProjector(JetHProjectors.HistProjector):
...
class JetHResponseMatrix(object):
    ...
    def __init__(self, configFile, productionRootFile, collisionSystem = None, useFloatHists = None, outputPath = None, responseMatrixBaseName = None, clusterBias = None, *args, **kwargs):
        ...
        if collisionSystem == None:
            collisionSystem = self.config.get("collisionSystem", str(JetHUtils.CollisionSystem.kNA))
        if collisionSystem[:1] != "k":
            collisionSystem = "k" + collisionSystem
        self.collisionSystem = JetHUtils.CollisionSystem[collisionSystem]
    ...
    def Initialize(self):
        ...
        unmatchedDetLevelJetSpectraProjector.additionalAxisCuts.append(HistAxisRange(axisType = JetResponseMakerJetsSparse.kLeadingParticlePbPb if self.collisionSystem == JetHUtils.CollisionSystem.kPbPb else JetResponseMakerJetsSparse.kLeadingParticlePP,
    ...
    def ProjectPostScaledAndMergedHists(self):
        ...
        partSpectraProjector.additionalAxisCuts.append(HistAxisRange(axisType = JetHProjectors.TH1AxisType.xAxis,
        ...
        partSpectraProjector.projectionAxes.append(HistAxisRange(axisType = JetHProjectors.TH1AxisType.yAxis,
    ...
    def PlotHists(self, plotIndividualPtHardBins = True, responseMatrixSaveName = "responseMatrix"):
        ...
        if plotIndividualPtHardBins:
            ...
            if self.ptHardBinProduction:
                # Pt Hard Spectra
                self.hists["ptHardSpectra"].SetTitle("p_{T} Hard Spectra")
                self.hists["ptHardSpectra"].GetXaxis().SetTitle("p_{T}^{Hard}")
                self.hists["ptHardSpectra"].GetYaxis().SetTitle("#frac{dN}{dp_{T}^{Hard}}")
                ...
                self.hists["ptHardSpectraAfterEventSelection"].SetTitle("p_{T} Hard Spectra After Event Selection")
                self.hists["ptHardSpectraAfterEventSelection"].GetXaxis().SetTitle("p_{T}^{Hard}")
                self.hists["ptHardSpectraAfterEventSelection"].GetYaxis().SetTitle("#frac{dN}{dp_{T}^{Hard}}")
        ...
        latex.append(ROOT.TLatex(.58, .72, "20 < p_{T}^{det} < 40 GeV/#it{c}"))
    ...
    def RetrieveHists(self):
        ...
        for ptHardBin in ptHardBins:
            ...
            if pythiaInfoTaskName:
                ...
                pythiaInfo = JetHUtils.getHistogramsInList(filename, pythiaInfoTaskName)
                pythiaInfoAfterEventSelection = JetHUtils.getHistogramsInList(filename, taskConfiguration["pythiaInfoAfterEventSelectionTaskName"])
            # Get the response maker
            responseMakerHists = JetHUtils.getHistogramsInList(filename, taskConfiguration["responseMakerTaskName"])
            # Get the particle level sample task (to check the jet spectra independently)
            sampleTaskParticleLevel = JetHUtils.getHistogramsInList(filename, taskConfiguration["sampleTaskParticleLevelTaskName"])
            ...
            sampleTaskDetLevel = JetHUtils.getHistogramsInList(filename, taskConfiguration["sampleTaskDetLevelTaskName"])
    ...
    def ExtractScaleFactors(self):
        ...
        for ptHardBin, xSecHist, nTrialsHist in zip(self.hists[xSecListName].keys(), self.hists[xSecListName].values(), self.hists[nTrialsListName].values()):
            ...
            if self.collisionSystem == JetHUtils.CollisionSystem.kpp and ptHardBin == "9":
        ...
        for x in range(1, responseMatrix.GetXaxis().GetNbins() + 1):
            for y in range(1, responseMatrix.GetYaxis().GetNbins() + 1):
                ...
                if responseMatrix.GetBinContent(x, y) > 0:
                    ...
                    if responseMatrix.GetBinError(x, y) > JetHUtils.epsilon:
    ...
    def createResponseMatrixAnalysis(jetHArgs):
        ...
        """
        # Define the proper response
        if jetHArgs.get("testMatrix", False):
            jetHArgs["testMatrixWeight"] = args.testMatrixWeight
            JetHResponse = JetHTestResponseMatrix(**jetHArgs)
        elif jetHArgs.get("embeddingExamplePlots", False):
            JetHResponse = JetHEmbeddingToyExample(**jetHArgs)
        else:
...
class JetHEmbeddingToyExample(JetHResponseMatrix):
    def __init__(self, collisionSystem, productionRootFile, minValue = 30, maxValue = 50):
        super(JetHEmbeddingToyExample, self).__init__(collisionSystem, productionRootFile)
        self.minValue = 30
        self.maxValue = 50
    ...
    def CreateEmbeddingExampleProjections():
        ...
        histName = "truthLevelProjectionForDetPt{0:.0f}"
        for ptBinValue in range(self.minValue, self.maxValue):
            ptBin = self.hists["responseMatrix"].GetXaxis().FindBin(ptBinValue)
            # Projecting a given det level pt onto the truth axis
            ptYProjection = self.hists["responseMatrix"].ProjectionY(histName.format(float(ptBinValue)), ptBin, ptBin)
        ...
        self.hists["embeddingExamplePlotsDetLevel"] = detLevel

        truthLevel = collections.OrderedDict()
        histName = "detLevelProjectionForTruthPt{0:.0f}"
        for ptBinValue in range(self.minValue, self.maxValue):
            ptBin = self.hists["responseMatrix"].GetYaxis().FindBin(ptBinValue)
            # Projecting a given det level pt onto the truth axis
            ptXProjection = self.hists["responseMatrix"].ProjectionX(histName.format(float(ptBinValue)), ptBin, ptBin)
            ...
            # Titles
            ptXProjection.SetTitle("Detector level jet p_{T} for truth level jets")
            ptXProjection.GetYaxis().SetTitle("Probability")
            #ptXProjection.GetXaxis().SetRangeUser(0, 70)

            # Save in dict
            truthLevel[ptBinValue] = ptXProjection

        self.hists["embeddingExamplePlotsTruthLevel"] = truthLevel
    ...
    def PlotEmbeddingExamplePlots(canvas):
        ...
        # Get input hists
        inputDictDetLevel = self.hists["embeddingExamplePlotsDetLevel"]
        ...
        #############################################
        # Draw projections for adjacent bins together
        # Det level selected
        #############################################
        drawFlag = False
        ...
        # Save canvas
        canvas.SaveAs(os.path.join(outputPath, "detectorPtExample.pdf"))

        # Also create a linear y axis plot for comparison
        canvas.SetLogy(False)
        # Save canvas
        canvas.SaveAs(os.path.join(outputPath, "detectorPtExampleNoLog.pdf"))
        canvas.SetLogy(True)

        #############################################
        # Draw projections for adjacent bins together
        # Truth level selected
        #############################################
        ...
        for (label, hist), color in zip(inputDictTruthLevel.iteritems(), plottingUtils.colorSchemes.sequential + plottingUtils.colorSchemes.qualitative):
            ...
            hist.GetXaxis().SetTitle("p_{T}^{det} (GeV/c)")

            # Add legend entry
            leg.AddEntry(hist, "p_{{T,jet}}^{{truth}} = {0:.1f}".format(label))
        ...
        # Save canvas
        canvas.SaveAs(os.path.join(outputPath, "truthPtExample.pdf"))

        # Also create a linear y axis plot for comparison
        canvas.SetLogy(False)
        # Save canvas
        canvas.SaveAs(os.path.join(outputPath, "truthPtExampleNoLog.pdf"))
        canvas.SetLogy(True)

        # Cleanup
        # Reset line width
        for hist in itertools.chain(inputDictDetLevel.values(), inputDictTruthLevel.values()):
            hist.SetLineWidth(1)
        ...
        # Draw a line represnting the "label" GeV det level jet
        canvas.Update()
        canvas.Modified()
        ...
        #################################
        # Draw selections of a particular projection to show how the embedding would work
        #################################
        # Setup canvas
        canvas.SetLogy(True)
        ...
        # Arbitrarily selected selection range
        ...
        for iSelectionBin in range(0, len(selections)-1):
            ...
            # Enable to save the canvas to check that the filling works
            #canvas.SaveAs(os.path.join(outputPath, "truthPtSelections_SelectionBin{0}.pdf".format(iSelectionBin)))
        ...
        # Save canvas
        canvas.SaveAs(os.path.join(outputPath, "truthPtSelections.pdf"))
...
class JetHTestResponseMatrix(JetHResponseMatrix):
    def __init__(self, collisionSystem, productionRootFile, weight):
        super(JetHTestResponseMatrix, self).__init__(collisionSystem, productionRootFile, responseMatrixBaseName = "testMatrix")
        self.weight = weight
        # Don't normalize if we've intentionally set a different weight
        if self.weight != 1:
    ...
    def CreateTestResponseMatrix(self, weight):
        ...
        #    responseMatrix.Fill(i, i)
        #    # Half weight for testing purposes
        #    #matrix.Fill(i, i, 0.5)

        # This will work as long as there are the same number of bins in x and y
        # That should normally be the case!

        responseMatrix = self.hists["responseMatrix"]
        for i in range(1, responseMatrix.GetXaxis().GetNbins() + 1):
            # Find the bin associated with each value
            #x = responseMatrix.GetXaxis().FindBin(i)
            #fillLocation = responseMatrix.GetXaxis().GetBinCneter(x)
            fillLocation = responseMatrix.GetXaxis().GetBinCenter(i)

            responseMatrix.Fill(fillLocation, fillLocation, self.weight)
...
def plot1DPtHardHists(full, ptHardList, canvas, outputPath, ptHardBinning = []):
    ...
    if ptHardBinning:
        ...
        if ROOT.gROOT.GetVersionInt() > 60000:
            legend.SetHeader("p_{T} bins", "C")
        else:
            legend.SetHeader("p_{T} bins")
...
def validateArguments(args):
    ...
    if args.get("collisionSystem", False):
        try:
            ...
            args["collisionSystem"] = JetHUtils.CollisionSystem["k" + args["collisionSystem"]]
        except KeyError:
